...
`ENABLE_METADATA` This option allows you to enable scanning the metadata of your files. If this is enabled, the file will be parsed and tested with TMDB, with TorBox Search as a fallback, to __attempt__ to find the correct metadata for your files in your TorBox account. This isn't perfect, but runs well with American content & Anime. If this option is `false` it skips scanning and places all of your video files in the `movies` folder. If enabled, files will be scanned and placed into either the `movies` or `series` folders. Please also keep in mind that you will be subject to rate limiting of the TMDB discover endpoint & TorBox's search endpoint when using the metadata option. It is generally recommended to keep this option enabled, if possible, for the best out-of-the-box experience. Also keep in mind, this unlocks the `instant` option, which can allow you to refresh every 6 minutes.
