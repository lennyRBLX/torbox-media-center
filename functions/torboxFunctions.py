...
from functions.mediaFunctions import (
    constructSeriesTitle,
    cleanTitle,
    cleanYear,
    normaliseTitle,
    scoreTmdbResult,
    TMDB_SCORE_THRESHOLD,
)
...
diag_log = logging.getLogger("tmdb")
...
_RE_TRAILING_ROMAN = re.compile(
    r"\s+(?:i{1,3}|iv|v(?:i{0,3})|ix|x(?:i{0,3}))$", re.IGNORECASE
)
...
# Anime non-episode artifacts: NCOP/NCED with optional digits/version, or bare OP/ED
# only when followed by digits. Boundary lookarounds prevent matches inside words
# like OPENING/EDITION.
NON_EPISODE_TAG_RE = re.compile(
    r"(?:^|[\s\-_.\[(])"
    r"(?:NC(?:OP|ED)\d*|(?:OP|ED)\d+)"
    r"(?:v\d+)?"
    r"(?=[\s\-_.\])]|$)",
    re.IGNORECASE,
)
...
SKIP_SUBFOLDERS = frozenset(
    {"extras", "specials", "op", "ed", "ncop", "nced", "featurettes"}
)

# --- Diagnostics log file ---
DIAG_LOG_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "tmdb_diagnostics.log"
)
_diag_file_handler: logging.FileHandler | None = None
...
def _classifyQueryHint(item_name: str, file_name: str) -> str:
    """Return 'tv', 'movie', or 'both' based on filename patterns + PTN parse."""
    text = f"{item_name} {file_name}"
    if _RE_TV_INDICATOR.search(text):
        return "tv"
    parsed = PTN.parse(file_name) if file_name else PTN.parse(item_name)
    if isinstance(parsed.get("season"), (int, list)) or isinstance(
        parsed.get("episode"), (int, list)
    ):
        return "tv"
    if parsed.get("year"):
        return "movie"
...
def _mergeQueryHint(prev: str | None, new: str) -> str:
    """Merge hints across multiple files for same normalised query.
    'both' wins over conflicting tv/movie; otherwise consistent labels stick."""
    if prev is None:
        return new
    if prev == new:
        return prev
...
def _loadKnownStableKeys(download_type: str) -> set[str]:
    ...
    q = Query()
    with db_lock:
        return {r["stable_key"] for r in db.search(q.stable_key.exists())}


_METADATA_CACHE_PATH = os.path.join(
...
def _enterBatchMode():
    ...
    file_size = (
        os.path.getsize(_METADATA_CACHE_PATH)
    ...
    if os.path.exists(_METADATA_CACHE_PATH):
        try:
            with open(_METADATA_CACHE_PATH, encoding="utf-8") as f:
                payload = json.load(f)
            now = int(time.time())
            for record in payload.get("_default", {}).values():
                raw_records += 1
            ...
            log.warning(f"_enterBatchMode: failed to read {_METADATA_CACHE_PATH}: {e}")

    log.info(
        f"_enterBatchMode: file_size={file_size}B raw_records={raw_records} "
        f"wrong_schema={wrong_schema} expired={expired} preloaded={len(_batch_cache)}"
    )
...
def _exitBatchMode():
    ...
    if not _batch_cache:
        log.warning("_exitBatchMode: _batch_cache is empty — skipping flush.")
        return

    payload = {
        "_default": {
            str(i + 1): record for i, record in enumerate(_batch_cache.values())
        }
    }
    tmp_path = _METADATA_CACHE_PATH + ".tmp"
    try:
        with open(tmp_path, "w", encoding="utf-8") as f:
            json.dump(payload, f)
            f.flush()
            os.fsync(f.fileno())
        os.replace(tmp_path, _METADATA_CACHE_PATH)
        written_size = os.path.getsize(_METADATA_CACHE_PATH)
        log.info(
            f"_exitBatchMode: wrote {len(_batch_cache)} entries to "
            f"{_METADATA_CACHE_PATH} ({written_size}B)"
        )
    except Exception as e:
        log.exception(f"_exitBatchMode: failed to write {_METADATA_CACHE_PATH}: {e}")
        try:
            os.remove(tmp_path)
        except OSError as cleanup_err:
            log.debug(
                f"_exitBatchMode: tmp cleanup failed path={tmp_path} err={cleanup_err}"
            )
        return

    from functions.databaseFunctions import db_connections, db_locks, global_lock

    with global_lock:
        db_connections.pop(METADATA_CACHE_DB_NAME, None)
        db_locks.pop(METADATA_CACHE_DB_NAME, None)

    _batch_cache = {}


def _reset_diag_log():
    global _cache_hits, _cache_misses, _diag_file_handler
    with _cache_counter_lock:
        _cache_hits = 0
        _cache_misses = 0
    if _diag_file_handler is not None:
        diag_log.removeHandler(_diag_file_handler)
        _diag_file_handler.close()
        _diag_file_handler = None
    if not TMDB_DIAG_ENABLED:
        return
    _diag_file_handler = logging.FileHandler(DIAG_LOG_PATH, mode="w", encoding="utf-8")
    _diag_file_handler.setFormatter(logging.Formatter("%(message)s"))
    diag_log.addHandler(_diag_file_handler)
    diag_log.setLevel(logging.INFO)
    diag_log.info(
        f"=== TMDB Diagnostics Log — {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')} ==="
    )


def _close_diag_log():
    global _diag_file_handler
    if _diag_file_handler is not None:
        diag_log.removeHandler(_diag_file_handler)
        _diag_file_handler.close()
        _diag_file_handler = None
...
METADATA_CACHE_SCHEMA_VERSION = 3
...
def getMetadataCacheKey(download_type: DownloadType, item: dict, file: dict):
    return f"v{METADATA_CACHE_SCHEMA_VERSION}:{download_type.value}:{item.get('hash')}:{file.get('id')}:{file.get('short_name') or file.get('name')}:{file.get('mimetype')}"
...
def getCachedMetadata(cache_key: str):
    ...
    with db_lock:
        ...
        now = int(time.time())
        if (
            record.get("schema_version") != METADATA_CACHE_SCHEMA_VERSION
...
def pruneExpiredMetadataCache():
    ...
    with db_lock:
        removed = db.remove(
            (query.schema_version != METADATA_CACHE_SCHEMA_VERSION)
            | (query.expires_at <= now)
        )
        if removed:
            log.info(f"Pruned {len(removed)} expired metadata cache entries.")
...
def process_file(item, file, type):
    ...
    if (
        not mimetype
        or not mimetype.startswith("video/")
        or mimetype not in ACCEPTABLE_MIME_TYPES
    ):
        log.debug(f"Skipping file {short_name} with mimetype {mimetype}")
        return None
    ...
    if NON_EPISODE_TAG_RE.search(file_stem_basename):
        diag(
            f"  SKIPPED (anime non-episode tag, torrent_id={torrent_id}): {short_name}"
        )
        return None
    ...
    if EXCLUDE_RESOLUTIONS:
        ...
        if m and f"{m.group(1)}p".lower() in EXCLUDE_RESOLUTIONS:
            diag(
                f"  SKIPPED (excluded resolution {m.group(1)}p, torrent_id={torrent_id}): {short_name}"
            )
            return None
    ...
    has_no_folder_context = (
        item_name == file_stem
        or item_name == short_name
        or item_name == item.get("hash")
    )
    if is_episode_only and has_no_folder_context:
        diag(
            f"  SKIPPED (episode-only, no folder context, torrent_id={torrent_id}): {short_name}"
        )
        return None

    file_path = file.get("name") or ""
    folder_name = (
        os.path.dirname(file_path) if "/" in file_path or "\\" in file_path else ""
    )
    ...
    if folder_name:
        ...
        if any(part.strip().lower() in SKIP_SUBFOLDERS for part in path_parts):
            diag(
                f"  SKIPPED (non-episode subfolder '{folder_name}', torrent_id={torrent_id}): {short_name}"
            )
            return None
...
def getUserDownloads(type: DownloadType):
    ...
    with timer("getUserDownloads.apiFetch", type=type.value) as fetch_fields:
        while True:
            params = {
                "limit": limit,
                "offset": offset,
            ...
            try:
                response = requestWrapper(
                    api_http_client, "GET", f"/{type.value}/mylist", params=params
                )
            except Exception as e:
                log.error(f"Error fetching {type.value} at offset {offset}: {e}")
                return (
                    None,
                    False,
                    f"Error fetching {type.value} at offset {offset}: {e}",
                )
            if response.status_code != 200:
                return (
                    None,
                    False,
                    f"Error fetching {type.value} at offset {offset}. {response.status_code}",
                )
            try:
                data = response.json().get("data", [])
            except Exception as e:
                log.error(f"Error parsing {type.value} at offset {offset}: {e}")
                log.error(f"Response: {response.text}")
                return (
                    None,
                    False,
                    f"Error parsing {type.value} at offset {offset}. {e}",
                )
            if not data:
                break
            ...
            if len(data) < limit:
                break
        fetch_fields["count"] = len(file_data)

    if not file_data:
        return None, True, f"No {type.value} found."

    log.debug(f"Fetched {len(file_data)} {type.value} items from API.")

    if SCAN_METADATA:
        with timer("getUserDownloads.pruneCache", type=type.value):
            pruneExpiredMetadataCache()

    with timer("getUserDownloads.loadKnownKeys", type=type.value) as kk_fields:
        known_keys = _loadKnownStableKeys(type.value)
        kk_fields["known"] = len(known_keys)
    files_to_process = []
    skipped_items = 0
    skipped_files = 0
    unchanged_hashes: set[str] = set()

    with timer("getUserDownloads.incrementalCheck", type=type.value) as inc_fields:
        for item in file_data:
            ...
            if all_known:
                ...
                if item_hash:
                    unchanged_hashes.add(item_hash)
                continue
        ...
        inc_fields["total"] = total_items
        inc_fields["unchanged"] = skipped_items
        inc_fields["changed"] = changed_items
        inc_fields["files_to_process"] = len(files_to_process)
        log.info(
    ...
    if not files_to_process:
        ...
        if cache_empty:
            log.warning(
                f"All {type.value} items unchanged but metadata cache is empty — forcing full reprocessing."
            )
            for item in file_data:
                if not item.get("cached", False):
                    continue
            ...
            log.info(f"All {type.value} items unchanged, skipping processing.")
    ...
    if SCAN_METADATA:
        with timer("getUserDownloads.enterBatchMode", type=type.value) as eb_fields:
            _enterBatchMode()
            eb_fields["preloaded"] = len(_batch_cache)

    try:
        if SCAN_METADATA and TMDB_API_KEY:
            log.info(f"Extracting unique titles from {len(files_to_process)} files...")
            query_hints: dict[str, str] = {}
            with timer(
                "getUserDownloads.extractTitles",
                type=type.value,
                files=len(files_to_process),
            ) as et_fields:
                seen_items: dict[str, tuple[str, str]] = {}
                for item, file in files_to_process:
                    item_id = item.get("id")
                    if item_id in seen_items:
                        continue
                    item_name = item.get("name", "")
                    sample_file_name = file.get("short_name") or file.get("name") or ""
                    if item_name == item.get("hash"):
                        item_name = PTN.parse(sample_file_name).get("title", sample_file_name)
                    seen_items[item_id] = (item_name, sample_file_name)

                for item_name, sample_file_name in seen_items.values():
                    parsed_title = PTN.parse(item_name).get("title", item_name)
                    normalised = normaliseTitle(parsed_title)
                    if not normalised:
                        continue
                    hint = _classifyQueryHint(item_name, sample_file_name)
                    prev = query_hints.get(normalised)
                    query_hints[normalised] = _mergeQueryHint(prev, hint)
                et_fields["items"] = len(seen_items)
                et_fields["unique"] = len(query_hints)

            total_queries = len(query_hints)

            with timer("getUserDownloads.predictCacheCoverage", type=type.value) as cc_fields:
                predicted_misses = sum(
                    1 for item, file in files_to_process
                    if getMetadataCacheKey(type, item, file) not in _batch_cache
                )
                cc_fields["files"] = len(files_to_process)
                cc_fields["misses"] = predicted_misses
                cc_fields["hits"] = len(files_to_process) - predicted_misses

            if predicted_misses == 0 and total_queries > 0:
                log.info(
                    f"Skipping prefetch: 100% metadata cache coverage "
                    f"({len(files_to_process)} files, {total_queries} queries would be wasted)"
                )
                with timer("getUserDownloads.prefetch", type=type.value) as pf_fields:
                    pf_fields["queries"] = total_queries
                    pf_fields["skipped_warm_cache"] = True
                    pf_fields["endpoints_called"] = 0
                    pf_fields["endpoints_skipped"] = 0
            else:
                log.info(
                    f"Prefetching TMDB searches for {total_queries} unique titles with {PREFETCH_WORKERS} threads "
                    f"(predicted {predicted_misses} cache misses)..."
                )
                prefetch_ok = 0
                prefetch_fail = 0
                prefetch_endpoints_called = 0
                prefetch_endpoints_skipped = 0
                prefetch_start = time.time()
                prefetch_done = 0

                def _prefetch_one(query_with_idx):
                    idx, query, hint = query_with_idx
                    ok = True
                    endpoints = ("tv",) if hint == "tv" else ("movie",) if hint == "movie" else ("tv", "movie")
                    skipped = 2 - len(endpoints)
                    for search_type in endpoints:
                        try:
                            resp = requestWrapper(
                                tmdb_http_client,
                                "GET",
                            ...
                            diag(
                                f"  Prefetch [{idx}/{total_queries}] {search_type} '{query}' (hint={hint}) → {result_count} results"
                            )
                        except Exception as e:
                            diag(
                                f"  Prefetch [{idx}/{total_queries}] {search_type} '{query}' (hint={hint}) → FAILED: {e}"
                            )
                            ok = False
                    return ok, len(endpoints), skipped

                with timer("getUserDownloads.prefetch", type=type.value) as pf_fields:
                    with ThreadPoolExecutor(max_workers=PREFETCH_WORKERS) as pf_executor:
                        futures = {
                            pf_executor.submit(_prefetch_one, (i, q, query_hints[q])): i
                            for i, q in enumerate(query_hints.keys(), 1)
                        }
                        for future in as_completed(futures):
                            prefetch_done += 1
                            try:
                                ok, called, skipped = future.result()
                                prefetch_endpoints_called += called
                                prefetch_endpoints_skipped += skipped
                                if ok:
                                    prefetch_ok += 1
                                else:
                                    prefetch_fail += 1
                            except Exception as prefetch_err:
                                log.exception(f"prefetch failed: {prefetch_err}")
                                prefetch_fail += 1

                            if prefetch_done % 25 == 0 or prefetch_done == total_queries:
                                elapsed = time.time() - prefetch_start
                                log.info(
                                    f"Prefetch progress: {prefetch_done}/{total_queries} ({elapsed:.1f}s elapsed)"
                                )

                    pf_fields["queries"] = total_queries
                    pf_fields["ok"] = prefetch_ok
                    pf_fields["failed"] = prefetch_fail
                    pf_fields["workers"] = PREFETCH_WORKERS
                    pf_fields["endpoints_called"] = prefetch_endpoints_called
                    pf_fields["endpoints_skipped"] = prefetch_endpoints_skipped

        total_files = len(files_to_process)
        log.info(f"Processing {total_files} files with {max_workers} threads...")
        process_start = time.time()
        completed = 0
        skipped = 0
        errors = 0

        with timer("getUserDownloads.process", type=type.value) as proc_fields:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(future_to_file):
                    ...
                    try:
                        ...
                        log.error(
                            f"Error processing file {file.get('short_name', 'unknown')}: {e}"
                        )
            ...
            proc_fields["total"] = total_files
            proc_fields["matched"] = len(files)
            proc_fields["skipped"] = skipped
            proc_fields["errors"] = errors
            proc_fields["workers"] = max_workers
            with _cache_counter_lock:
                proc_fields["cache_hits"] = _cache_hits
                proc_fields["cache_misses"] = _cache_misses

        if files:
            log.info(f"Writing {len(files)} records to {type.value} DB...")
            with timer("getUserDownloads.write", type=type.value, count=len(files)):
                batchUpsertData(files, type.value, ["stable_key"])

        if unchanged_hashes:
            with timer(
                "getUserDownloads.includeUnchanged", type=type.value
            ) as iu_fields:
                db = getDatabase(type.value)
                db_lock = getDatabaseLock(type.value)
                added = 0
                if db is not None and db_lock is not None:
                    with db_lock:
                        all_rows = db.all()
                    seen_keys = {f["stable_key"] for f in files if f.get("stable_key")}
                    for row in all_rows:
                        if (
                            row.get("folder_hash") in unchanged_hashes
                            and row.get("stable_key")
                            and row["stable_key"] not in seen_keys
                        ):
                            files.append(dict(row))
                            added += 1
                iu_fields["unchanged_items"] = len(unchanged_hashes)
                iu_fields["added"] = added
                log.info(
                    f"Re-included {added} unchanged DB rows from "
                    f"{len(unchanged_hashes)} unchanged items (stale-purge protection)."
                )
    finally:
        if SCAN_METADATA:
            log.info("Flushing metadata cache...")
            with timer("getUserDownloads.exitBatchMode", type=type.value) as xb_fields:
                xb_fields["flushed"] = len(_batch_cache)
                _exitBatchMode()
            _close_diag_log()
            with _cache_counter_lock:
            ...
            log.info(
                f"Metadata cache stats after {type.value}: hits={hits} misses={misses} ({hit_pct}% hit rate)"
            )
...
def searchTMDB(
    ...
    for current_query in search_queries:
        ...
        for search_type, media_type in search_order:
            ...
            try:
                ...
                log.warning(
                    f"TMDB search error for '{current_query}' ({search_type}): {e}"
                )
                continue

            if response.status_code != 200:
                log.warning(
                    f"TMDB search returned {response.status_code} for '{current_query}' ({search_type})"
                )
                continue
            ...
            if parsed_year and candidates:
                for candidate in candidates:
                    ...
                    partial = fuzz.partial_ratio(
                        normalised_input, normaliseTitle(candidate_title)
                    )
                    if partial >= TITLE_DETAIL_THRESHOLD and media_type == "tv":
                        tmdb_id = candidate.get("id")
                        try:
                            detail_resp = requestWrapper(
                                tmdb_http_client,
                                "GET",
                                f"/tv/{tmdb_id}",
                            ...
                            log.debug(f"TMDB detail fetch failed for tv/{tmdb_id}: {e}")

            for result in candidates:
                score, breakdown = scoreTmdbResult(
                    title,
                    parsed_year,
                    parsed_season,
                    parsed_episode,
    ...
    diag_lines = [
        ...
        + (
            f"  [override: PTN had season={ptn_season} episode={ptn_episode}, matched '{override_match_text}' in file name]"
    ...
    for i, bd in enumerate(sorted(all_scored, key=lambda x: x["total"], reverse=True)):
        ...
        tied = (
            " [TIED]" if bd is not best_breakdown and bd["total"] == best_score else ""
        )
        yr_start = bd.get("tmdb_year") or "-"
        yr_end = bd.get("tmdb_year_end")
        airing = (
            f"{yr_start}-{yr_end}" if yr_end and yr_end != yr_start else str(yr_start)
        )
        diag_lines.append(
    ...
    if not accepted:
        diag_lines.append(
            f"  RESULT: NO MATCH (best score {best_score} < threshold {TMDB_SCORE_THRESHOLD})"
        )
        diag("\n".join(diag_lines))
        return None
    ...
    if is_single_result and best_score < TMDB_SCORE_THRESHOLD:
        diag_lines.append(
            f"  RESULT: ACCEPTED (single result) '{best_breakdown['tmdb_title']}' (score {best_score} >= single-result threshold {SINGLE_RESULT_THRESHOLD})"
        )
    else:
        diag_lines.append(
            f"  RESULT: ACCEPTED '{best_breakdown['tmdb_title']}' (score {best_score} >= threshold {TMDB_SCORE_THRESHOLD})"
        )
    diag("\n".join(diag_lines))

    tmdb_title = cleanTitle(
        best_result.get("title") or best_result.get("name") or title
    )
    date_str = (
        best_result.get("release_date") or best_result.get("first_air_date") or ""
    )
    tmdb_year = None
    if date_str:
        try:
    ...
    metadata = {
        ...
        "metadata_image": f"https://image.tmdb.org/t/p/w500{poster}"
        if poster
        else None,
        "metadata_backdrop": f"https://image.tmdb.org/t/p/w1280{backdrop}"
        if backdrop
        else None,
        ...
        "metadata_rootfoldername": f"{tmdb_title} ({tmdb_year})"
        if tmdb_year
        else tmdb_title,
    ...
    if best_media_type == "tv":
        series_season_episode = constructSeriesTitle(
            season=parsed_season, episode=parsed_episode
        )
        if series_season_episode:
            metadata["metadata_filename"] = (
                f"{tmdb_title} {series_season_episode}{extension}"
            )
        metadata["metadata_foldername"] = constructSeriesTitle(
            season=parsed_season, folder=True
        )
...
def searchMetadata(
    ...
    def cacheAndReturn(
        metadata: dict, success: bool, detail: str, failure_kind: str | None = None
    ):
        if cache_key is not None:
            setCachedMetadata(
                cache_key, metadata, success, detail, failure_kind=failure_kind
            )
    ...
    if cache_key is not None:
        ...
        if cached_result is not None:
            ...
            log.debug(f"Metadata cache hit for key {cache_key}")
            _bump_cache_hit()
            return (
                cached_metadata,
                cached_success,
                f"Metadata cache hit. {cached_detail}",
            )
        _bump_cache_miss()
    ...
    if tmdb_result is not None:
        ...
        return cacheAndReturn(
            base_metadata,
            True,
            f"TMDB match via torrent name (score: {tmdb_result.get('metadata_tmdb_score')}). Searching for {folder_query}, item hash: {hash}",
        )
    ...
    if (
        subfolder_query
        and normalised_subfolder
        and normalised_subfolder != normalised_folder
    ):
        diag(
            f"  >> Retrying TMDB with subfolder name: '{subfolder_query}' (normalised: '{normalised_subfolder}', torrent_id={torrent_id})"
        )
        ...
        if tmdb_result is not None:
            ...
            return cacheAndReturn(
                base_metadata,
                True,
                f"TMDB match via subfolder name (score: {tmdb_result.get('metadata_tmdb_score')}). Searching for {subfolder_query}, item hash: {hash}",
            )
    ...
    is_numeric_only = bool(
        normalised_file_query and _RE_NUMERIC_ONLY.match(normalised_file_query)
    )
    already_tried = normalised_file_query in (normalised_folder, normalised_subfolder)
    if (
        file_query
        and not starts_with_episode_tag
        and not is_numeric_only
        and not already_tried
    ):
        diag(
            f"  >> Retrying TMDB with file name: '{file_query}' (normalised: '{normalised_file_query}', torrent_id={torrent_id})"
        )
        tmdb_result = searchTMDB(
            file_query,
            title_data,
        ...
        if tmdb_result is not None:
            ...
            return cacheAndReturn(
                base_metadata,
                True,
                f"TMDB match via file name (score: {tmdb_result.get('metadata_tmdb_score')}). Searching for {file_query}, item hash: {hash}",
            )
    else:
        skip_reasons = []
        if not file_query:
        ...
        if already_tried:
            skip_reasons.append(
                f"same as already-tried query ('{normalised_file_query}')"
            )
        diag(
            f"  >> File fallback SKIPPED (torrent_id={torrent_id}): {', '.join(skip_reasons) or 'unknown'}"
        )

    diag(
        f"  >> Falling back to TorBox Search API for: {file_name} (torrent_id={torrent_id})"
    )
    extension = os.path.splitext(file_name)[-1]
    try:
        response = requestWrapper(
            search_api_http_client,
            "GET",
            f"/meta/search/{full_title}",
            params={"type": "file"},
        )
    except Exception as e:
        log.error(f"Error searching metadata: {e}")
        return cacheAndReturn(
            base_metadata,
            False,
            f"Error searching metadata: {e}. Searching for {query}, item hash: {hash}",
            failure_kind="transient",
        )
    if response.status_code != 200:
        log.error(f"Error searching metadata: {response.status_code}. {response.text}")
        kind = (
            "transient"
        ...
        return cacheAndReturn(
            base_metadata,
            False,
            f"Error searching metadata. {response.status_code}. Searching for {query}, item hash: {hash}",
            failure_kind=kind,
        )
    try:
        ...
        if data.get("type") == "anime" or data.get("type") == "series":
            ...
            base_metadata["metadata_foldername"] = constructSeriesTitle(
                season=title_data.get("season"), folder=True
            )
            ...
            return cacheAndReturn(
                base_metadata,
                False,
                f"No metadata found. Searching for {query}, item hash: {hash}",
        ...
        base_metadata["metadata_rootfoldername"] = (
            f"{title} ({base_metadata['metadata_years']})"
        )

        return cacheAndReturn(
            base_metadata,
            True,
            f"Metadata found. Searching for {query}, item hash: {hash}",
        )
    except IndexError:
        return cacheAndReturn(
            base_metadata,
        ...
        return cacheAndReturn(
            base_metadata,
            False,
            f"Timeout searching metadata. Searching for {query}, item hash: {hash}",
        )
    except Exception as e:
        log.error(f"Error searching metadata: {e}")
        log.error(f"Error searching metadata: {traceback.format_exc()}")
        return cacheAndReturn(
            base_metadata,
            False,
            f"Error searching metadata: {e}. Searching for {query}, item hash: {hash}",
...
def getDownloadLink(url: str):
    ...
    if (
        response.status_code == httpx.codes.TEMPORARY_REDIRECT
        or response.status_code == httpx.codes.PERMANENT_REDIRECT
        or response.status_code == httpx.codes.FOUND
    ):
